...
# Update Cell 7 (Regression)
for cell in nb['cells']:
    if "### 7. Difference-in-Differences (DiD) Regression" in "".join(cell.get('source', [])):
        cell['source'] = [
...
# Update Cell 9 (Heterogeneity)
for cell in nb['cells']:
    if "### 9. Heterogeneity Analysis: Urban vs. Rural" in "".join(cell.get('source', [])):
        cell['source'] = [
            ...
            "df_urban = df_panel[df_panel['is_urban'] == 1].dropna(subset=reg_vars)\n",
            "df_rural = df_panel[df_panel['is_urban'] == 0].dropna(subset=reg_vars)\n",
            "\n",
...
# Update Cell 11 (Robustness)
for cell in nb['cells']:
    if "### 11. Robustness Check: Leave-One-County-Out" in "".join(cell.get('source', [])):
        cell['source'] = [
            ...
            "    df_sub = df_panel[df_panel['REGCNTY'] != county].dropna(subset=reg_vars)\n",
...
# Update Cell 12 (Mechanism)
for cell in nb['cells']:
    if "### 12. Mechanism Check: The Schooling Proxy" in "".join(cell.get('source', [])):
        cell['source'] = [
...
print("Notebook updated with NaN handling fixes.")
