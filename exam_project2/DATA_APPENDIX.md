...
### 4.6 Multi-census religion and sex data (POP1880, POP1885, POP1890) — appendix-only

The Galloway Prussian county-level files contain religion and sex breakdowns at every census between 1871 and 1910. The analysis panel uses only the 1871 cross-section (REL1871 §4.3 and POP1871 §4.4); the 1880, 1885, 1890 religion and sex columns are loaded **only by ad-hoc scripts that produce [`outputs/tables/catholic_share_stability.tex`](outputs/tables/catholic_share_stability.tex)**, the appendix table that validates the time-invariance of `cath_share` and the demographic balance of the two treatment groups across the sample period. These variables are deliberately *not* merged into `analysis_panel.parquet` because no headline regression uses them.

| Census | File | Catholic column(s) used | Population column(s) used | Notes |
|---|---|---|---|---|
| 1871 | `REL1871.xlsx` (§4.3) | `Relcathm + Relcathf` | `Pop` (REL1871), `Popm + Popf` (POP1871) | Religion and sex pulled from separate files. |
| 1880 | `POP1880.XLS` | `Relcath` (total only — male/female not separately reported) | `Pop`; sex from `Poplivingm + Poplivingf` (de facto population) | The 1880 census tabulates Catholics in a single aggregate column; sex is pulled from the de-facto-present count rather than de-jure to match the religion tabulation convention. |
| 1885 | `POP1885.XLS` | `RELCATH` (total) | `POP`, `POPM`, `POPF` | The 1885 file additionally reports `RELCATHM` (male Catholic), allowing derivation of `RELCATHF = RELCATH - RELCATHM`. We use the total directly. |
| 1890 | `POP1890.XLS` | `RELCATHM + RELCATHF` | `POPM + POPF` (no aggregate `POP` in the source file) | The 1890 file does not contain an aggregate `POP` column; total population is reconstructed as the sum of the two sex columns. |

**Derived quantities** (computed ad-hoc; not on the panel):

| Quantity | Formula | Used in |
|---|---|---|
| `cath_share_{year}` | $100 \cdot \text{Catholics}_t / \text{Pop}_t$ | Panels A, B, C of [Table~\ref{tab:catholic_share_stability}](outputs/tables/catholic_share_stability.tex) — Prussia-wide level, within-county group means, within-county persistence correlations. |
| `sex_ratio_{year}` | $100 \cdot \text{Pop}^M_t / \text{Pop}^F_t$ | Panel D of the stability table — men per 100 women by group. |
| Total pop by group | $\sum_{i \in g} \text{Pop}_{i,t}$ | Panel E — 1871→1890 cumulative growth and annualised growth rate. |

**Sample definitions used in the table.** The four-census *balanced* sample is the intersection of counties present in all four censuses with non-missing Catholic share: $N=474$ (cath\_share-based, Panels B/C) or $N=476$ (pop-based, Panel E). Within Panels B and C, eight counties whose 1871–1890 Catholic-share change exceeds $\pm 17$pp are dropped as 1881 Prussian *Kreis* boundary-reorganisation artefacts (verified to appear in administrative split/merge pairs), leaving $N=467$.

**Definitional consistency.** "Roman Catholic" is counted under the same definition at all four censuses. The post-1870 Old Catholic schism (which followed the First Vatican Council's 1870 declaration of papal infallibility) produced a small dissident population tabulated separately in 1890 as `RELDISSIDENTM + RELDISSIDENTF`; it is not aggregated into the Catholic counts here. Old Catholics never exceeded 0.1% of the Prussian population.

**Why these variables are not on `analysis_panel.parquet`.** The headline DiD specification treats `cath_share` as a time-invariant 1871 covariate (§6.1, §9 caveat). The multi-census religion columns enter the project only to validate that assumption empirically; persisting them as panel columns would invite their use as time-varying treatment intensity, which is not the identification strategy. The sex-ratio columns are similarly only descriptive (Panel D documents that the cross-group sex ratio is small and stable). Future specifications that want to exploit the 1871→1890 within-county variation in cath\_share (e.g.\ a Bartik-style instrument or a triple difference with a measured-conversion channel) would need to formally merge these into the panel via a new loader in [`src/data/load_data.py`](src/data/load_data.py).
...
|---|---|---|---|---|
| `cath_share` | Catholic population share, 1871 (Galloway-derived; time-invariance empirically validated in §4.6 / [Table~\ref{tab:catholic_share_stability}](outputs/tables/catholic_share_stability.tex) against POP1880, POP1885, POP1890 — within-county $\rho = 0.998$ over 19 years) | $(\texttt{Relcathm}+\texttt{Relcathf})/\texttt{Pop}\times 100$ | % (0–100), time-invariant | [`load_data.py:102`](src/data/load_data.py:102) |
...
- **`cath_share` is time-invariant from 1871** ([`load_data.py:102`](src/data/load_data.py:102)). The implicit assumption is that the *cross-section* of Catholic shares is stable across 1862–1890. *Empirically validated*: against POP1880, POP1885, POP1890 (§4.6), within-county Catholic-share persistence is $\rho = 0.9997$ over 1871–1880 (Kulturkampf-enforcement decade) and $\rho = 0.9982$ over the full 1871–1890 window; the within-county standard deviation of 1871→1880 changes is 0.87pp; aggregate Catholic share drifted upward by only 0.66pp across the 19-year sample, with the (small) high-Catholic decline larger during the 1880–1890 rollback than during 1871–1880 enforcement — ruling out a conversion-response interpretation. See [Table~\ref{tab:catholic_share_stability}](outputs/tables/catholic_share_stability.tex), Panels A–C. The same table additionally documents that sex ratios are balanced across treatment groups (Panel D — high-Catholic counties have $\approx$ 0.6 *more* men per 100 women than low-Catholic, the wrong sign and too small to explain the 4–5\% lower Catholic marriage rate) and that the Polish-Catholic provinces' net out-migration is the entire source of the modest aggregate growth-rate gap between high- and low-Catholic counties (Panel E).
