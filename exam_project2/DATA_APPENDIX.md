...
| `infant_mortality_rate` | Infant deaths per 1,000 *legitimate* live births | $\texttt{Dth\_infant\_leg}/\texttt{Birlegtot}\times 1000$ if $\texttt{Birlegtot}>0$, else NaN | per 1,000; **analysis restricted to 1875+** | [`build_dataset.py:177–185`](src/data/build_dataset.py:177) |
| `infant_mortality_rate_total` | **Total** infant deaths per 1,000 *total* live births | $(\texttt{Dth\_infant\_leg}+\texttt{Dth\_infant\_bas})/(\texttt{Birlegtot}+\texttt{Birbastot})\times 1000$ if both numerator terms present and denominator $>0$, else NaN | per 1,000; **1875+ only** (pre-1875 the illegitimate-infant-death column `Dth<1bas` does not exist in any Galloway VIT file, so this rate is undefined). On the current build, mean is ~10 per 1,000 *higher* than `infant_mortality_rate` because illegitimate-infant mortality was systematically higher than legitimate-infant mortality. | [`build_dataset.py:188–199`](src/data/build_dataset.py:188) |
...
- **Infant-mortality definition changes in 1875.** `Dth_infant_leg` falls back from `Dth<1leg` (true legitimate-infant deaths under 1 year) to `Dthyoung` (broader / less complete young-deaths category) when the former is absent ([`load_data.py:325–334`](src/data/load_data.py:325)). The empirical signature of this measurement change is a sharp ~3–4× jump in mean IMR at 1875 (see [`fig_imr_break.png`](outputs/figures/fig_imr_break.png) and [`fig_imr_by_group.png`](outputs/figures/fig_imr_by_group.png)). Analyses using `infant_mortality_rate` are therefore restricted to 1875+ ([`channels.py:80`](src/analysis/channels.py:80)). The companion **total**-IMR variable `infant_mortality_rate_total` (legitimate + illegitimate infant deaths per 1,000 total live births) is, by construction, only defined for 1875+ because `Dth<1bas` does not appear in pre-1875 VIT files.
