...
| Variable | Definition | Formula | Unit | Built at |
|---|---|---|---|---|
| `ln_pop` | Log mid-year population (constructed but **not** a default regression control; retained as a column for descriptive use) | $\ln(\texttt{Poptot\_midyear})$ — uses the same mid-year denominator as the headline rates so the regression does not mix conventions | log persons | [`build_dataset.py:178–181`](src/data/build_dataset.py:178) |
...
- **Illegitimacy** ([`illegitimacy_analysis()`](src/analysis/channels.py:17)): baseline DiD on `illegitimacy_ratio` with the single regressor `cath_share_x_post` (entity + year FE; no time-varying controls).
